...
// Function nambahin rekening baru
const tambahRekening = async (req, res, next) => {
    try {
...
// Function nampilin daftar rekening
const daftarRekening = async (req, res, next) => {
    try {
...
// Function nampilin detail rekening berdasarkan ID
const detailRekeningById = async (req, res, next) => {
    try {
...
// Function nampilin detail rekening berdasarkan nama bank
const detailRekeningByBank = async (req, res, next) => {
    try {
        // Dapetin nama bank rekening dari parameter URL
        let namaBank = req.params.bankName;

        // Nyari rekening berdasarkan nama bank dengan nama pengguna terkait
        let rekening = await prisma.rekening.findMany({
            where: { namaBank },
            include: { pengguna: { select: { nama: true } } }
        });
        ...
        if (!rekening) {
            return res.status(404).json({
                status: false,
                message: `rekening di bank ${namaBank} nggak ada!`,
                data: null
            });
        }
